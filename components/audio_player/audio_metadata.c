...
#define ALBUM_IMG_SIZE 1024*1024
...
bool read_mp3_ID3V1(const char *mp3_file) {
    ...
    if (strncmp(buffer, "TAG", 3) == 0) {
        ...
        printf("artist:%s\n",id3v1_data.artist);
        printf("album:%s\n",id3v1_data.album);
        // strncpy(id3v1_data->Year, buffer + 93, 4);
        // strncpy(id3v1_data->Comment, buffer + 97, 28);
        // id3v1_data->Genre = buffer[127];
        // strncpy(tag->Genre, buffer + 127, 1);
        return true;
    } else {
...
bool read_mp3_ID3V2(const char *mp3_file) {
    ...
    bytes_read = fread(buffer, 1, 10, file);//读取10个字节写入到buffer中
